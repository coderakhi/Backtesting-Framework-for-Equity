...
import urllib
log = logging.getLogger("kiteconnect")
...
class KiteTicker(object):
    ...
    def __init__(self, username, access_token, debug=False, root=None,
        ...
        self.socket_url = "{root}?api_key=kitefront&{query}".format(root=self.root, query=urllib.parse.urlencode({
            "user_id": username,
            "enctoken": access_token,
            "uid": str(int(dt.datetime.now().timestamp() * 1000)),
